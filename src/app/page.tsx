...
import { Sun, Moon, Play, Pause, Music } from "lucide-react";
      ...
      <AnimatePresence mode="wait">
        ...
        {!isProfileVisible ? (
          ...
          >
            ...
            <motion.button
              className="mt-8 px-8 py-3 bg-gray-900 text-white dark:bg-white dark:text-gray-900 rounded-full font-semibold shadow-lg transition-transform"
          ...
          >
            ...
            <motion.div 
              className="fixed bottom-4 right-4 z-50 flex items-center gap-3 p-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md rounded-lg shadow-lg"
              initial={{ opacity: 0, x: 100 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 1, duration: 0.5 }} // Muncul setelah profil fade in
            >
              {/* Ikon Musik yang Berputar */}
              <Music 
                size={20} 
                className={ isPlaying ? "animate-spin" : "" } // Hanya berputar jika isPlaying true
              />
              
              {/* Judul Lagu (truncate jika terlalu panjang) */}
              ...
              <button onClick={togglePlayPause} className="p-2 rounded-full hover:bg-gray-300 dark:hover:bg-gray-700 transition">
                {isPlaying ? (
                  <Pause size={20} className="fill-current" /> // Tampil jika sedang play
                ) : (
                  <Play size={20} className="fill-current" /> // Tampil jika sedang pause
                )}
              </button>
            </motion.div>
